...
var C = require("../../dist/index.js");

var CALL = C.CALL
var Callable = C.default
...
function defaultTest(Class, instances = []) {
    return describe(getTitle(Class, true), function () {
        ...
        it('correctly inherits prototypes', function () {
            ...
            assert(typeof new Class('msg') === 'function')
            const InstancesDefault = [Object, Function, Callable, Class]
            for (let i = 0; i !== InstancesDefault.length; i++) {
                assert(new Class('msg') instanceof InstancesDefault[i])
            }
...
describe(getTitle(MyTestWithCall), function () {
    ...
    it('has own string tag Redefined', function () {
        assert(Object.prototype.toString.call(new MyTestWithCall('test')) === '[object Redefined]')
    })
    it("supports property redefine", function () {
    ...
    it('has only one [Callable.CALL] method', function () {
        assert(MyTestWithCall[CALL] === MyTestWithCall[Callable.CALL] && Callable.CALL === CALL)
    })
});
